...
BEGIN TRY
	INSERT INTO [dbo].[Ads]
	([Title], [DatePosted], [Description], [Price], [ClientName] ,[Address], [City], [ZIP], [Country], [Phone], 
	 [BrandId], [CarBodyTypeId], [CarConditionId], [ColorId], [FuelId], [ProductionYearId])
	VALUES
	(@Title, @DatePosted, @Description, @Price, @ClientName, @Address, @City, @ZIP, @Country, @Phone, 
	 @BrandId, @CarBodyTypeId, @CarConditionId, @ColorId, @FuelId, @ProductionYearId);
...
AS
	SET NOCOUNT ON;
BEGIN TRY
	SELECT [Id], [Title], [DatePosted], [Description], [Price], [ClientName], [Address], [City], [ZIP], [Country], 
		   [Phone], [BrandId], [CarBodyTypeId], [CarConditionId], [ColorId], [FuelId], [ProductionYearId]
	FROM [dbo].[Ads]
	WHERE [Id] = @Id;
	RETURN 0;
END TRY
